 ...
 */
static int globalize_page_range(struct arm_mmu_ptables *dst_pt, struct arm_mmu_ptables *src_pt, uintptr_t virt_start, size_t size, const char *name)
{
...
void arch_mem_domain_partition_remove(struct k_mem_domain *domain, uint32_t partition_id)
{
    ...
    remove_map(ptables, partition->name, partition->start, partition->size);
}
...
void arch_mem_domain_partition_add(struct k_mem_domain *domain, uint32_t partition_id)
{
    ...
    partition = &domain->partitions[partition_id];
    add_map(ptables, partition->name, partition->start, partition->start, partition->size, partition->attr);
}
...
void arch_mem_domain_destroy(struct k_mem_domain *domain)
{
    ...
    discard_page_range(ptables, 0, (uintptr_t)UINTPTR_MAX);
}

void arch_mem_domain_init(struct k_mem_domain *domain)
{
    struct arm_mmu_ptables *ptables = &domain->arch.ptables;
    ...
    for (int i = 0; i < CONFIG_MAX_DOMAIN_PARTITIONS; i++) {
        if (domain->partitions[i].size) {
            arch_mem_domain_partition_add(domain, i);
        }
    }

    sys_slist_append(&domain_list, &domain->node);
}
...
{
    thread->arch.ptables = &kernel_ptables;
}
