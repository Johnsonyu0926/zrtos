...
#define MAX_PORT_CNT         3
#define MAX_PIN_CNT_PER_PORT 6
...
struct mfd_it8801_data {
	...
	/* MFD sub device */
	const struct device *sub_dev_kbd;
	const struct device *sub_dev_gpio[MAX_PORT_CNT][MAX_PIN_CNT_PER_PORT];
	it8801_callback_handler_t mfd_cb[share_irq_dev_cnt];
};
...
void mfd_it8801_register_interrupt_callback(const struct device *mfd, const struct device *child,
					    ...
					    enum reg_irq_dev share_irq_name, uint8_t gpio_inst,
					    uint8_t idx)
{
	struct mfd_it8801_data *data = mfd->data;

	/* Save the sub node device of mfd for the callback function */
	if (share_irq_name == share_irq_kbd) {
		data->sub_dev_kbd = child;
		data->mfd_cb[share_irq_kbd] = cb;
	} else {
		data->sub_dev_gpio[gpio_inst][idx] = child;
		data->mfd_cb[share_irq_gpio] = cb;
	}
}
...
static void it8801_gpio_alert_worker(struct k_work *work)
{
	...
	if (data->mfd_cb[share_irq_kbd]) {
		data->mfd_cb[share_irq_kbd](data->sub_dev_kbd);
	}

	if (data->mfd_cb[share_irq_gpio]) {
		for (int i = 0; i < MAX_PORT_CNT; i++) {
			for (int j = 0; j < MAX_PIN_CNT_PER_PORT; j++) {
				if (data->sub_dev_gpio[i][j] != NULL) {
					data->mfd_cb[share_irq_gpio](data->sub_dev_gpio[i][j]);
				}
			}
		}
	}
