...
end_init_intn_gpio:
	...
	err = gpio_pin_configure_dt(&config->rstn_gpio, GPIO_OUTPUT_ACTIVE);
	if (err) {
...
#define GPIO_AW9523B_DEFINE(inst)                                                                  \
	...
	static const struct gpio_aw9523b_config gpio_aw9523b_config##inst = {                      \
		...
		IF_ENABLED(DT_INST_PROP_HAS_IDX(inst, rstn_gpios, 0), (                            \
			   .rstn_gpio = GPIO_DT_SPEC_INST_GET(inst, rstn_gpios),                   \
		))                                                                                 \
	};                                                                                         \
												   \
